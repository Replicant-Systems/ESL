...
// Wakeup time target (10:30 AM)
const int targetHour = 18;
    ...
    for (int i = 0; i < numberStr.length() && digitIndex < 4; i++) {
        ...
        if (c == '.') {
            if (digitIndex > 0 && digitIndex < 4) {
                segments[digitIndex - 1] |= 0b10000000;  // Decimal point
            }
        } else if (isdigit(c)) {
...
void setup() {
    ...
    if (timeClient.update()) {
        // Calculate time to next wakeup (10:30 AM)
        ...
        unsigned long sleepDuration = (targetEpoch - currentEpoch) * 1000000UL;  // Convert to microseconds
        Serial.print("currentEpoch :");
        Serial.print(currentEpoch);
        Serial.print(" ------- targetEpoch :");
        Serial.print(targetEpoch);
        ...
        fetchDataAndDisplay();
        ESP.deepSleep(sleepDuration);  // Go to deep sleep
    } else {
...
void loop() {
    // Loop is empty because the ESP8266 will sleep after setup
}
